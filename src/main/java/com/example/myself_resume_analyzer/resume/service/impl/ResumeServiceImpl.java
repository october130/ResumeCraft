...
import com.example.myself_resume_analyzer.common.constant.ErrorMessages;
...
import com.example.myself_resume_analyzer.common.utils.Md5Utils;
...
log.info("数据库里已经有用户的信息了，包括OSS");
    ...
    @Override
    public Result<String> reanalyze(Long resumeId, Long userId) {
        ResumeRecord record = resumeRecordMapper.selectById( resumeId);
        if (record == null) {
            return Result.error(ErrorMessages.RESUME_NOT_EXIST);
        }
        if (!record.getUserId().equals(userId)) {
            return Result.error("你的用户和简历登录用户不符合无权查看");
        }
        if (record.getParseStatus() != 3){
            return Result.error("只有失败状态下才可以重新进行简历解析");
        }
        resumeParseAsyncService.reanalyzeAsync(resumeId, userId);
        return Result.success("开始重新分析，请稍等一会");
    }

    @Override
    public byte[] exportPdf(Long resumeId, Long userId) {
        ResumeRecord record = resumeRecordMapper.selectById( resumeId);
        if (record == null) {
            throw new RuntimeException("简历不存在");
        }
        if (!record.getUserId().equals(userId)) {
          throw new RuntimeException("你的用户和简历登录用户不符合无权查看");
        }
    QueryWrapper<ResumeEvaluation> wrapper = new QueryWrapper<>();
        wrapper.eq("resume_id", resumeId).orderByDesc("analyzed_at").last("LIMIT 1");
        //首先去查简历表，之后再查评分表获取最新的分数
        ResumeEvaluation evaluation = resumeEvaluationMapper.selectOne(wrapper);
        if (evaluation == null){
            throw new RuntimeException("暂时还没有评分哦，没有办法导出PDF文件");
        }
      byte[]pdfFile = pdfExporterService.exportPdf(resumeId, record, evaluation);
        log.info("PDF导出文件文件已生成: {}", pdfFile);
        return pdfFile;
    }
